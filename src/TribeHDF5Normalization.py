...
class TribeHDF5Normalization:
    ...
    def executer_pipeline(self, tribe_hdf5=None, cneuromod_hdf5=None):
        ...
        try:
            ...
            dataset_cneuromod = cneuromod_hdf5[self.cneuromod_ses][
                self.cneuromod_dataset
            ][:]
            ...
            if self.flag_delai_bold_brute:
                if self.centrage_donne_temps:
                    temps_source = (
                        np.arange(dataset_tribe_bonne_duree.shape[0]) * self.t_Tribe_s
                        + self.t_Tribe_s / 2
                    )
                    temps_cible = (
                        np.arange(self.Y_cible.shape[0]) * self.TR_irmf_s
                        + self.TR_irmf_s / 2
                    )
                ...
                dataset_tribe_propre = convolve(
                    dataset_tribe_bonne_duree, noyau_hrf, mode="full"
                )
                dataset_tribe_propre = dataset_tribe_propre[:nb_instants_valides, :]

                # 4. Création des axes temporels
                temps_source = (
                    np.arange(dataset_tribe_propre.shape[0]) * self.t_Tribe_s
                    + self.t_Tribe_s / 2
                )
                temps_cible = (
                    np.arange(self.Y_cible.shape[0]) * self.TR_irmf_s
            ...
            aligneur_temporel = interp1d(
                temps_source,
                dataset_tribe_propre,
                kind="previous",
                axis=0,
                bounds_error=False,
                fill_value=0.0,
...
if __name__ == "__main__":
    ...
    chemin_video = (
        DATA_DIR / "sub-01" / "ses-001" / "sub-01_ses-001_task-thingsmemory_run-1.mp4"
    )
    ...
    if flag_precision_voxel:
        chemin_cneuromod = (
            ...
            / "voxel_native"
            / "sub-03"
            / "sub-03_task-things_space-T1w_desc-voxelwise_timeseries.h5"
        )
    else:
        chemin_cneuromod = (
            ROOT
            ...
            / "cneuromod2026"
            / "sub-03"
            / "sub-03_task-things_space-MNI152NLin2009cAsym_atlas-cneuromod26_desc-1134Parcels_timeseries.h5"
        )
